...
class lensingClass(object):
	...
	#oi = open('integrand_components.txt', 'a')
        n_save = self.n_i(zprime, numList, orgList, zlist)
 ...
 #               print "In calcIntegrationArray, checking z: ", zval
                ...
                outarray.append(self.lensingWeightFunctionIntegrand(zprime, z, numList, orgList, zlist)) 
...
#	f4.close()
	...
	if self.units == 1:
        ...
        integrand = self.calcIntegrationArray(zlim, orgList, numList, lensingzlist)
	...
	#f1.close()
        ...
        W *= 3/2*omegamat*100*100*self.h*self.h*D_A*W/300000.
        print "W is ", W
	...
	for z in orgList:
		...
		nl.append(300000*pow(z, 2)*math.exp(-pow(z/.35, 1.2)))
	...
	for zt in zlist:
		...
		if zt >= orgList[savedi] - delta and zt < orgList[savedi+1] + delta:
			...
			#f3.write('i: ' + str(i) + '\t' + 'orgList[i]: ' + str(orgList[i]) + '\t' + 'orgList[i+1]: ' + str(orgList[i+1]) + '\n')
			...
			#f3.write(str(x_prime) + '\t' + str(self.gaussFit(x_prime, z)) + '\t')
	...
	#f2.write('n num: ' + str(numList[savedi]/265601.*1./(orgList[1] - orgList[0])) + '\t\t' + 'n: ' + str(150000*pow(z, 2)*math.exp(-pow(z/.5, 1.2))/20770.8/5.0) +  '\t' + 'galaxynum: ' + str(galaxynum) + '\t' + 'z: ' + str(z) + '\t' + 'int: ' + '\t' + str(n_z) + '\n\n')
	#f2.close()

        #return numList[savedi]/265600.*n_z
	return numList[savedi]/(orgList[1] - orgList[0])/max(orgList)*n_z/265601.0#self.numgalaxies
